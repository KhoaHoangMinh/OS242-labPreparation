...
void* worker_thread(void* arg){	//tac vu muon lam
...
void kill_thread (int id){
	...
	if (id >= 0 && id < count){
		...
		printf("B: Killed thread %d\n", id);
	}
	//pthread_mutex_unlock (&manager.lock);
}

void cleanup() {
	printf("B is quitting...\n");
...
void* manager_thread (void* arg) {
	int fd = open(FIFO_TO_B, O_RDONLY);
	char command[50];

	while (1){
		if (read(fd, command, sizeof(command)) > 0){ 
			if (strncmp (command, "CREATE_THREAD", 13)==0) create_thread();
			else if (strncmp (command, "LIST_THREADS", 12)==0) list_threads();
			else if (strncmp (command, "EXIT", 4)==0) cleanup();
...
void send_command (const char* cmd){
	int fd = open (FIFO_TO_A, O_WRONLY);
